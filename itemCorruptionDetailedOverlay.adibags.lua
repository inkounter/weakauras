...
-- On Init
-- NOTE: This implementation is forked specifically for AdiBags because of its
-- behavior.  In particular, it does not create the frame for the bag until it
-- is opened for the first time.
--
-- For the original implementation, see instead: https://wago.io/XsOUJBjhP
...
aura_env.acceptBankFrameOpenedEvent = false
...
aura_env.isBankBag = function(bag)
    -- Return 'true' if the specified 'bag' is in the bank.  Otherwise, return
    -- 'false'.

    return bag == -1 or bag > NUM_BAG_SLOTS
end

aura_env.getBagSlotFrameName_AdiBags = function(bag, slot)
    -- Return the name for the AdiBags 'Frame' element for the specified 'bag'
    -- and 'slot'.  If 'bag' and 'slot' are invalid, or if the AdiBags 'Frame'
    -- element does not yet exist, return 'nil' instead.

    local container
    if aura_env.isBankBag(bag) then
    ...
    return container.buttons[(bag * 100 + slot)]:GetName()
end
...
aura_env.getSlotFrameName = function(location)
    ...
    if location:IsBagAndSlot() then
        ...
        local frameName = aura_env.getBagSlotFrameName_AdiBags(bag, slot)
        if frameName == nil then
            -- AdiBags doesn't create its frames until they're opened for the
            -- first time.  Here, we assume that this is why the frame is not
            -- yet drawn (rather than assuming that we're using another bag
            -- addon/frame.

            -- Schedule another update for 'location' later.  Use a different
            -- interval for bank vs. backpack locations.

            local interval
            if aura_env.isBankBag(bag) then
                interval = aura_env.config.bankInterval
            else
                interval = aura_env.config.backpackInterval
            end

            C_Timer.After(interval,
                          function() WeakAuras.ScanEvents("WA_DEFERRED_CORRUPTION_OVERLAY_UPDATE", location) end)
        end
        return frameName
    elseif location:IsEquipmentSlot() then
...
function(allstates, event, arg1)
    ...
    elseif event == "BANKFRAME_OPENED" then
        ...
        if not aura_env.acceptBankFrameOpenedEvent then
            return false
        else
    ...
    elseif event == "OPTIONS" then
        -- The user opened the WeakAura Options window or is viewing the
        -- options for this WeakAura.

        -- Reject 'BANKFRAME_OPENED' events until 'dummy' is received.
        ...
        aura_env.updateStatesForLocations(allstates, locations)
        return true
    elseif event == "dummy" then
        -- This WeakAura is being initialized.

        -- Accept 'BANKFRAME_OPENED' events.  Note that this imposes a
        -- requirement that 'dummy' be fired after the dummy
        -- 'BANKFRAME_OPENED' event, which means that 'dummy' must be listed
        -- after 'BANKFRAME_OPENED' in the WeakAura's EVENTS list.

        aura_env.acceptBankFrameOpenedEvent = true
